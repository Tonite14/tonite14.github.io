...
同步情况下我们要使用一个值，我们当然会想到return，但在异步情况下，setTimeout在同步执行栈(call stack）里执行完毕后，除了将一个任务插入下一个事件循环的宏任务队列外，就无法在同步执行栈中提供任何可返回值了。
...
function double(value, callback) {
  ...
  //                     ↑ 这里调用回调，把结果 value*2 喂给它
}

double(3, (x) => console.log(`I was given: ${x}`));
//         ↑ 这个 (x) => ... 就是"要用到结果的那段代码"，包成了回调
...
回调模式的根本局限在于：发起异步调用时，必须同步绑定处理逻辑。异步结果不由函数返回，而是通过回调参数注入，仅在该回调的执行上下文中有效，时机一过即不可追溯。

#### 嵌套异步回调：

这一局限的根源在于内存模型的差异。回调模式下，异步结果仅作为实参短暂存在于一次函数调用的栈帧中。`success(2 * value)` 执行完毕，`success` 函数返回，栈帧弹出，结果值即被覆盖销毁。整个过程未将结果写入任何堆内存中的持久化数据结构，因此调用方无法在事后重新访问该值。

Promise 逆转这一点的机制本质上是将结果从栈上的临时参数迁移为堆上的对象属性。调用 `resolve(value)` 时，Promise 实例内部执行三个步骤：将 `[[PromiseState]]` 从 `"pending"` 置为 `"fulfilled"`；将 `value` 写入 `[[PromiseResult]]` 内部槽位；将此前通过 `.then()` 注册的回调以微任务形式排入事件队列。关键在第二步：结果值被持久化为 Promise 对象的一个内部属性。该对象存在于堆内存中，只要调用方持有其引用，`[[PromiseResult]]` 便始终可访问，不受任何栈帧销毁的影响。

>这个顺序可以略加解释一下，因为回调其实就是原来嵌套模式的回调，嵌套模式采用回调和数据同时绑定的模式，而promise则应当先把状态和值先写好，以方便then里面的回调取用。

因此 `.then(callback)` 在两种时机下均能正确工作。若调用时 `[[PromiseState]]` 仍为 `"pending"`，Promise 将 callback 存入内部列表 `[[PromiseFulfillReactions]]`，待未来 `resolve` 触发时取出执行。若调用时已为 `"fulfilled"`，Promise 直接从 `[[PromiseResult]]` 中取出已存储的值，将 callback 包装为微任务排入队列。无论哪种路径，调用方都无需在异步操作发起的同一时刻绑定处理逻辑：值已经安全地保存在对象内部，随时等待消费。

> 对于then可能创造的链式调用而言，promise对象和值的传递分为两个过程：
>
> - 同步阶段：一堆 pending 的容器被创建、传递、串联，形成流水线
> - 微任务阶段：值从上游灌进来，沿途经过每一道 `.then()` 加工，依次灌满下游容器
>
> 同步代码看见的永远是一排空瓶子，真正的水要到微任务队列开门才流进来。

简言之，回调模式要求调用方在异步发起的瞬间同步提交处理函数，因为值只存在于一次函数调用的参数传递中，没有独立于调用栈的存储位置。Promise 则为异步结果分配了独立的堆内存储存，将值和处理逻辑解耦为两个独立的时刻。


如果异步返值又依赖另一个异步返回值，那么回调的情况还会进一步变复杂。考虑一个典型场景：获取用户信息，然后依次获取其首篇文章、首条评论、评论者头像。每一步都依赖前一步的结果，每一步都是异步操作。在回调模型下，代码结构如下：

```js
function getUser(id, success, failure) {
  setTimeout(() => {
    if (id > 0) success({ id, name: 'Alice' });
    else failure('User not found');
  }, 500);
}

function getFirstPost(userId, success, failure) {
  setTimeout(() => {
    if (userId > 0) success({ postId: 101, title: 'Hello World' });
    else failure('No posts');
  }, 500);
}

function getFirstComment(postId, success, failure) {
  setTimeout(() => {
    if (postId > 0) success({ commentId: 201, body: 'Nice!' });
    else failure('No comments');
  }, 500);
}

function getCommenterAvatar(commentId, success, failure) {
  setTimeout(() => {
    if (commentId > 0) success('/avatars/bob.png');
    else failure('Avatar not found');
  }, 500);
}

// 四层嵌套：每一层都是回调的形参，语义被缩进淹没
getUser(1,
  (user) => {
    getFirstPost(user.id,
      (post) => {
        getFirstComment(post.postId,
          (comment) => {
            getCommenterAvatar(comment.commentId,
              (avatar) => {
                console.log(`User ${user.name}, first post: ${post.title}, first comment: ${comment.body}, avatar: ${avatar}`);
              },
              (err) => console.error('Avatar error:', err)
            );
          },
          (err) => console.error('Comment error:', err)
        );
      },
...
// User Alice, first post: Hello World, first comment: Nice!, avatar: /avatars/bob.png
```

这段代码暴露了回调策略的全部结构性问题。第一，代码向右不断缩进，形成经典的"厄运金字塔"（pyramid of doom），逻辑被视觉噪声淹没。第二，每一步的错误处理都需要独立的 `failure` 回调参数，四个步骤产生四段错误处理分支，而实际开发中这些分支通常不会认真编写，导致错误静默丢失。第三，若需要在第二步和第三步之间插入新步骤，必须将整条链从中间拆开重写，编辑成本极高。第四，`user` 变量的作用域跨越四层嵌套，任何中间步骤都可以意外修改它，状态流通过非局部变量在深度嵌套的作用域链中隐式传播，不可追踪。

显然，随着代码越来越复杂，回调策略不具有扩展性。"回调地狱"这个称呼可谓实至名归。嵌套回调的代码维护起来就是噩梦。

关键在于异步操作没有函数返回值，只能通过推入消息队列里的回调函数传递结果，`getUser(id, ...)` 的返回值立即是 `undefined`，本身无法返回任何东西。
...
>`Promise.resolve()`  拥有幂等性的关键原因是 `Promise.then()` 默认会用 `Promise.resolve()` 来包装内部回调的返回值，如果返回值已经就是 `Promise` 对象，那么就会套娃，再往下传递之前，下面的函数还得多解一层。让下面的函数硬解也可以解，但显然幂等性省去很多心智负担。
...
> `reject` 不需要幂等性的原因 `reject` 不会返回一个 Promise 对象作为拒绝值，`Promise.reject()` 的实参在规范设计和工程实践中都特化为 Error 对象，永远不会出现期约对象被当作拒绝理由再解包的场景。所以幂等性对 `reject` 来说是一个不需要考虑的问题。
...
> `reject()` 的设计目的是**携带拒绝理由**，让链上的 `.catch()` 能拿到这个值做业务处理。浏览器控制台打印Error: bar，是默认的兜底设计。引擎发现一个 `rejected` 期约到微任务结束都没人接，就把它的 `reason` 读取出来，当作未捕获错误展示给开发者。
...
>onRejected方法几乎是和onFullfilled方法对称的，主要习惯差异是`onFulfilled` 通常叫 `v`（value），`onRejected` 通常叫 `e` 或 `err`（error），因为拒绝理由几乎总是 Error 对象。
...
其一，异步场景中的值到达时机。在真正的异步场景中（网络请求、计时器），`resolve(value)` 被调用的时刻与回调地狱中回调触发的时刻完全相同——Promise 没有让值提前到达。以 `fetch` 为例，回调地狱在响应到达时拿到 `response`，Promise 同样在响应到达时才调用 `resolve(response)`，二者时间线一致。区别在于：`new Promise(...)` 返回的那一刻即得到一个容器对象，即使状态仍为 `pending`、值尚未到达，该容器已可存入变量、作为参数传递、提前挂载多个 `.then()` 处理程序。等异步结果返回时，引擎将值写入早已准备好的 `[[PromiseResult]]` 槽位，然后按注册清单通知排队的回调。容器先到，值后到——这才是 Promise 与回调地狱的真正分界线。

其二，微任务队列并非伴随 Promise 同时出现。在 ES2015 引入 Promise 之前，`MutationObserver`（DOM4 规范，约 2013 年）已经使用微任务机制来异步批量处理 DOM 变更，其回调同样在当前宏任务结束后、下一宏任务开始前的微任务检查点执行。ECMAScript 在规范层定义了内部队列 `PromiseJobs`，浏览器将其映射到已有的微任务队列实现。Promise 对微任务机制的贡献在于将其标准化并推入主流视野，而非从零创造。

由此，Promise 的贡献可以拆为两条平行的线：语法层面，通过状态机容器模式将回调的嵌套结构改写为链式调用，降低心智负担；调度层面，将 `.then()` 处理程序的执行时机标准化为微任务——在当前宏任务结束后、下一宏任务开始前必然执行——为后续 `async/await` 的语义提供了事件循环层的基础。
...
当期约落定后，**.then() 的回调不会立即执行**，而是进入**微任务队列**（是立即进入），等同步代码全部跑完才轮到微队列和消息队列的处理。而setTimeout会进入延时队列，而微队列的微任务一定比其他消息队列的宏任务先执行。
...
| **进入哪个队列** | 除微任务外的其余消息队列（Message Queue）                 | 微任务队列（Microtask Queue）                                |
...
>
> 每个宏任务（无论来自同步代码还是 setTimeout）都拥有自己独立的一轮，setTimeout 的注册属于当前轮，每轮宏任务（同步代码）执行完后，立即清空微任务队列；之后事件循环按优先级**从各个 Task Queue 里取下一个任务**，取到的那一个才作为"下一轮宏任务"执行。
>
