...
在这个内容开始前，应该先补充事件循环的部分知识，例如JS是一门单线程的语言，这是因为它运行在浏览器的渲染主线程中，而渲染主线程只有一个。每一个页面运行一套自己的js运行时，对应的主线程只能有一个，且一定不能暂停。
...
当期约落定后，**.then() 的回调不会立即执行**，而是进入**微任务队列**（是立即进入），等同步代码全部跑完才轮到它。而setTimeout会先进入延时队列，再进入微队列，在同步代码的宏任务进行时，无论是微队列还是其余消息队列都没有开始调用，只能将任务插入队列，那么一定是.then()先于setTimeout插入到微任务队列。
...
>
> 更准确地说，本质是事件循环在清空微任务之后，会主动去各个 Task Queue 里按优先级取任务。事件循环从 Task Queue 中按优先级取出下一个可执行的任务，才决定下一轮宏任务是否发生、以及发生哪一个。
>
