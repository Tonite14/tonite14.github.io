...
>
>margin collapsing 只在**正常流（normal flow）的 block 元素**之间生效。Flex 容器里的子项（flex items）虽然看起来也是"盒子排在一起"，但它们的布局模型已经不是 normal flow 了，margin 完全独立，上下左右各管各的。
...
> |                       | 比例                                                | 黑边                     |
> | :-------------------- | :-------------------------------------------------- | :----------------------- |
> | **大平台**（YouTube） | iframe在被返回时就已硬编码 16:9，平台内部做好自适应 | 无                       |
> | **自己平台的数据**    | 后端传 ratio 字段，动态设                           | 无                       |
> | **完全未知**          | 只能猜比例，比如 16:9                               | 很可能有，且不可单方消除 |
>
