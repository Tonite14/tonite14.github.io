...
这里需要澄清一个容易误判的细节：`[[Scope]]` 在内部函数定义时就已经指向了外层活动对象，但这不等于外层活动对象一定不会被回收。定义在嵌套作用域中但从未逃逸到外部的内部函数，GC 在标记清除时不会将其视为"从根可达"的节点，内外层函数的活动对象将被一并回收。

```
外层函数返回前                              外层函数返回后（内部函数未逃逸）
┌─────────────────────┐                    ┌────────────────────────┐
│ 外层活动对象         │ ← inner.[[Scope]] │ 外层活动对象 (?)        │
│  ├ inner            │──────────────────→│  ├ inner → 匿名函数     │
...
闭包真正阻止回收的条件是内部函数必须"逃逸"到外层作用域之外：被 `return`、赋值给外部变量、注册为事件回调，或者被塞入一个外部数据结构。只有逃逸后，从根对象（全局对象或调用栈）出发才能沿着引用链到达内部函数，进而通过 `[[Scope]]` 触及外层活动对象，使其免于回收。所谓闭包保留外层作用域，本质是内部函数作为桥梁，让外层活动对象所在的引用链变成了从根可达路径上的一环。
...
> 闭包会保留其外部函数的活动对象（变量对象），而非保留外部函数本身。外部函数的执行上下文在返回时即从调用栈弹出，代码逻辑不再可用，仅数据层因引用存活。所以比其他函数更占用内存。过度使用闭包可能导致内存过度占用，因此建议仅在十分必要时使用。V8等优化的JavaScript引擎会努力回收被闭包困住的内存，不过我们还是建议在使用闭包时要谨慎。
