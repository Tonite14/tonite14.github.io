...
## **map，set，weakmap和weakset的基本api对比：**

| 操作           | Map             | Set            | WeakMap         | WeakSet         |
| :------------- | :-------------- | :------------- | :-------------- | :-------------- |
| **创建**       | `new Map()`     | `new Set()`    | `new WeakMap()` | `new WeakSet()` |
| **添加/设置**  | `set(key, val)` | `add(val)`     | `set(key, val)` | `add(val)`      |
| **获取**       | `get(key)`      | ❌              | ❌               | ❌               |
| **是否存在**   | `has(key)`      | `has(val)`     | `has(key)`      | `has(val)`      |
| **删除**       | `delete(key)`   | `delete(val)`  | `delete(key)`   | `delete(val)`   |
| **清空**       | `clear()`       | `clear()`      | ❌               | ❌               |
| **长度**       | `size`          | `size`         | ❌               | ❌               |
| **键类型限制** | 任意值          | 任意值         | **仅对象**      | **仅对象**      |
| **可迭代**     | ✅               | ✅              | ❌               | ❌               |
| **entries()**  | ✅ `[key, val]`  | ✅ `[val, val]` | ❌               | ❌               |
| **keys()**     | ✅               | ✅ (= values)   | ❌               | ❌               |
| **values()**   | ✅               | ✅              | ❌               | ❌               |
| **forEach()**  | ✅               | ✅              | ❌               | ❌               |
