...
| 操作             | Object                 | Map             | Set                           | WeakMap         | WeakSet         |
| :--------------- | :--------------------- | :-------------- | :---------------------------- | :-------------- | :-------------- |
| **创建**         | `{}` / `new Object()`  | `new Map()`     | `new Set()`                   | `new WeakMap()` | `new WeakSet()` |
| **添加/设置**    | `obj[key] = val`       | `set(key, val)` | `add(val)`                    | `set(key, val)` | `add(val)`      |
| **获取**         | `obj[key]`             | `get(key)`      | ❌                             | `get(key)`      | ❌               |
| **是否存在**     | `key in obj`           | `has(key)`      | `has(val)`                    | `has(key)`      | `has(val)`      |
| **删除**         | `delete obj[key]`      | `delete(key)`   | `delete(val)`                 | `delete(key)`   | `delete(val)`   |
| **清空**         | 手动遍历删除           | `clear()`       | `clear()`                     | ❌               | ❌               |
| **长度**         | `Object.keys().length` | `size`          | `size`                        | ❌               | ❌               |
| **键类型限制**   | 字符串/Symbol          | 任意值          | 任意值                        | 仅对象          | 仅对象          |
...
- **Map**：key 可以是任意类型（对象、字符串、数字……）
- **WeakMap**：key **只能是对象**（不能用原始类型）

**2. 垃圾回收**

- **Map**：key 是强引用，意思是只要 Map 存在，key 指向的对象就不会被 GC
- **WeakMap**：key 是**弱引用**，即对象没有被其他地方引用时，GC 可以直接回收它，WeakMap 中对应的条目自动消失

这就是为什么 Vue3 响应式系统用 `WeakMap` 存 `target → depsMap`：当组件卸载、响应式对象被销毁，以它为 key 的整条依赖关系会自动被 GC 清理，不会内存泄漏。

**3. 不可迭代**

- **Map**：可遍历（`for...of`、`.keys()`、`.values()`、`.entries()`、`.size`）
- **WeakMap**：**不可遍历**，没有 `size`、没有 `.keys()`，因为条目随时可能被 GC 悄悄删掉，遍历结果不可靠

> WeakMap 在前端生态里最知名的用例就是 Vue3 的响应式依赖收集。
>
> 同类模式也出现在 Svelte 和 MobX 的响应式系统中，Web Components 的 `CustomElementRegistry` 同样依赖 WeakMap 绑定 DOM 节点与组件类，节点销毁后映射自动清理。
>
> 共同特征都是同一个场景：**给一个对象挂点额外数据，但对象销毁时数据要跟着自动消失。**
...
WeakMap中“weak”表示弱映射的键是“弱弱地拿着”的。意思就是，这些键不属于正式的引用，不会阻止垃圾回收。**WeakMap 的键值对能不能存在，只在于键（对象）有没有被别处强引用。只要键有被强引用，键-值对就会存在于映射中，并被当作对值的引用，因此就不会被当作垃圾回收**。
...
> 这种情况`obj` 变量持有对 `{ name: "Matt" }` 这个对象的强引用。即使 WeakMap 的键是弱引用，`obj` 也在"正式地拿着"这个对象，所以垃圾回收器不会动这个对象， WeakMap 里 `{ name: "Matt" } → "val"` 这条记录也保持完好。
>
> ```js
> const wm = new WeakMap();
...
但当把 `obj` 置空时，此时 `{ name: "Matt" }` 失去了所有外部的强引用。唯一还引用它的只剩 WeakMap 的键的弱引用。但弱引用不影响 GC 直接回收这个对象。对象没了，WeakMap 中对应的键值对也就自动消失了。
...
这也意味着**所有这些类型都兼容扩展操作符**。扩展操作符在**对可迭代对象执行浅复制时特别有用**，只需简单的语法就可以复制整个对象：
