...
这四项限制共享同一个底层原因：箭头函数不创建自己的 `this` 绑定、不持有自己的 `arguments` 对象、不实现 `[[Construct]]` 内部方法，因此也不具备与之联动的 `prototype` 属性。需要区分的是，箭头函数在调用时仍会创建执行上下文并推入调用栈，拥有独立的栈帧和局部变量对象。缺失的是上述四项与普通函数身份绑定的机制，而非执行上下文本身。

> 关于箭头函数 `this` 的深入辨析（调用时 vs 定义时、词法作用域继承），见另一篇专题：[2026-05-08-this-analysis.md](2026-05-08-this-analysis)——里面有完整的对比表、设计原理和常见踩坑场景。
