...
结果直接报权限错误——GitHub Actions Bot 对 `gh-pages` 分支没有 write 权限（仓库设置了分支保护）。

## 坑四：head.html 覆盖 → 暗色模式失效

最终想到的方案是：创建 `_includes/head.html` 覆盖主题自带的版本，在 `<head>` 里注入自定义 CSS。

但 Chirpy 的 `compress_html` 配置（`clippings: all, endings: all`）会自动剥离 `</head>` 标签，导致后续的 `mode-toggle.html` include 内容丢失——**暗色模式切换按钮直接消失了**。
...
一开始试过 `#main-wrapper::before` + `z-index: -1`，但 Chirpy 给 `#main-wrapper` 设了 `background-color`，会直接盖住 `z-index: -1` 的伪元素。放在 `html` 上是最可靠的方式——**没有任何元素能在 html 下面**。
