---
title: 阅读DAY17 JavaScript高级程序设计 12章上 BOM
...
| 重要度 | 知识点                                                       | 理由                                                         |
| :----: | :----------------------------------------------------------- | :----------------------------------------------------------- |
|  ⭐⭐⭐   | `var` vs `let`/`const` 挂载 `window` 的区别                  | 面试高频基础题，"var和let声明全局变量有什么区别"几乎必问     |
|  ⭐⭐⭐   | `window.innerWidth` vs `document.documentElement.clientWidth`（桌面等同、移动端可见视口/布局视口区分） | 响应式开发核心，移动端适配必知，面试常追问"移动端两套视口区别" |
|  ⭐⭐⭐   | 可见视口 vs 布局视口（缩放行为不同：pinch-zoom 只缩可见视口，布局视口不变） | 移动端适配最重要的概念，原书有误需纠正，面试可能问"移动端缩放哪些值会变" |
|  ⭐⭐⭐   | `window.open()` 弹窗屏蔽检测（返回 `null` + `try/catch` 双重覆盖） | 实际开发会遇到，面试经典题"如何检测弹窗被拦截"               |
|  ⭐⭐⭐   | `setTimeout()` 递归替代 `setInterval()`（确保前一次执行完才排期下一次） | 面试高频"为什么不用setInterval"，生产代码必备模式            |
|  ⭐⭐⭐   | 定时器第二参数是等待时间而非精确执行时间（任务队列机制）     | 事件循环核心概念，面试常出执行顺序题，“setTimeout(fn, 0)真的0ms执行吗” |
|  ⭐⭐⭐   | `location.assign()` / `location.href` / `location.replace()` / `location.reload()` | 路由跳转开发高频使用，replace vs assign 区别面试可问         |
|  ⭐⭐⭐   | `history.pushState()` / `replaceState()` / `popstate` 事件   | SPA 路由核心原理，Vue Router / React Router 底层都靠它，面试必问 |
|  ⭐⭐⭐   | `devicePixelRatio`（物理像素与 CSS 像素的缩放系数）          | 高清屏适配必备，1px 边框问题、图片清晰度问题都依赖此概念     |
|   ⭐⭐   | `globalThis`（跨环境统一全局对象访问）                       | 了解即可，知道浏览器中等价 `window`，实际开发中很少显式使用  |
|   ⭐⭐   | `top` / `parent` / `self` 窗口层级关系                       | iframe 通信场景必知，面试可能问"iframe中怎么拿到顶层窗口"    |
|   ⭐⭐   | `window.open()` 四参数 + `target` 行为（`_self`/`_blank`/窗口名） | 知道 `_self` 等价 `location.assign()` 即可，实际开发中 `window.open()` 使用频率不高 |
|   ⭐⭐   | `opener` 属性与切断（`opener = null` 允许独立进程）          | 安全相关，`<a rel="noopener">` 的 JS 版本，防 tab-nabbing 攻击 |
|   ⭐⭐   | `scrollTo()` / `scrollBy()` + `ScrollToOptions`（`behavior: 'smooth'`） | 实际开发中滚动控制常用，smooth scroll 用户体验优化           |
|   ⭐⭐   | `location` 属性修改 URL（`hash` 不重载、其他属性重载）       | SPA hash 路由基础，知道 hash 改变不刷新页面即可              |
|   ⭐⭐   | `history.go()` / `back()` / `forward()` / `length`           | 基础导航 API，知道有这些方法即可                             |
|   ⭐⭐   | `pushState()` 的假 URL 需真实后端对应（否则刷新 404）        | SPA 部署经典坑，面试可能问"Vue Router history 模式刷新 404 怎么解决" |
|   ⭐⭐   | `alert`/`confirm`/`prompt` 同步模态 + `this` 在定时器回调中的指向 | 知道它们是同步阻塞的，实际开发中几乎不用（用自定义弹窗），但面试可能问区别 |
|   ⭐⭐   | `window` 双重身份（ECMAScript Global 对象 + 浏览器窗口接口） | 概念性知识，理解 JS 运行环境的基础                           |
|   ⭐    | `window` 安全检测变量（`window.oldValue` 不报错）            | 小技巧，面试几乎不考，实际开发也很少用                       |
|   ⭐    | `screenLeft` / `screenTop` + `moveTo()` / `moveBy()`         | 现代浏览器基本禁用窗口移动，实际开发不会用                   |
|   ⭐    | `outerWidth` / `outerHeight`                                 | 几乎不用，知道和 inner 的区别即可                            |
|   ⭐    | `resizeTo()` / `resizeBy()`                                  | 同 moveBy，现代浏览器基本禁用                                |
|   ⭐    | `window.open()` 特性字符串（第三个参数）                     | 现代浏览器大部分选项已不支持，了解即可                       |
|   ⭐    | `pageXOffset` / `pageYOffset`（已废弃，用 `scrollX`/`scrollY`） | 知道用新 API 即可                                            |
|   ⭐    | 浏览器防滥用机制（连续弹窗出现复选框）                       | 了解即可，不影响开发                                         |
|   ⭐    | `find()` / `print()` 异步对话框                              | 几乎不用，知道是异步的即可                                   |
|   ⭐    | `setInterval()` 语法细节                                     | 已被 setTimeout 递归模式替代，了解接口即可                   |
|   ⭐    | `go()` 传入字符串导航                                        | 旧特性，现代浏览器不支持                                     |
|   ⭐    | `replaceState()` 更新当前状态不增历史记录                    | 低频使用，知道有这个方法即可                                 |
...
关键点：**间隔时间指的是向队列添加新任务之前等待的时间**，而不是两次执行之间的间隔。比如 01:00:00 调用，间隔 3000ms，则 01:00:03 添加任务、01:00:06 再添加任务，浏览器不管前一个任务是否执行完。因此，**执行时间短、非阻塞的回调**才适合 `setInterval()`。
...
`setInterval()` 在生产环境中很少使用，因为**两次执行之间的实际间隔无法保证**，某些循环任务可能会被跳过。用 `setTimeout()` 递归调用可以实现同样的效果，且能确保前一次执行完才排期下一次：
...
- **两次独立用户操作**各自触发一个对话框 → 不会出现复选框
- **一次用户操作**连续触发多个对话框 → 第二个起出现复选框
...
用户在浏览器菜单上选择“查找”(find)和“打印”(print)时显示的就是这两种对话框。与上面三种同步对话框不同，这两个是**异步**的，调用后控制权立即返回脚本。
