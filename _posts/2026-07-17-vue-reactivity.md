...
Vue 3 的响应式系统是整个框架的运转核心。为什么改了数据，页面就自动更新？`ref` 和 `reactive` 到底做了什么？模板里用到的变量为什么能精确追踪，没用到的不触发更新？

这篇文章从零拆解 Vue 3 响应式系统的四个核心概念——`Proxy`、`effect`、`track`、`trigger`——并附上一份 200 行的最小化实现，把"魔法"还原成可运行的 JavaScript。
