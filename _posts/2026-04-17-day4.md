...
```javascript
// 思路2：和nums[right]比较（变化基准）→ 不能跳过mid
var findMin = function(nums) {
    let left = 0;
    ...
    while (left < right) { // < 而不是 <=
        ...
        if (nums[mid] > nums[right]) {
        	left = mid + 1; // mid一定不是答案，敢跳
        } else {
        	right = mid; // mid可能是答案，不能跳过
        }
    }
    return nums[left];
};
...
但要注意一点，**比较基准在数组中变化时，不能套左闭右闭跳过元素的模板。** 因为`right`在收缩，`nums[mid] <= nums[right]`不能确定mid不是答案，必须`right = mid`保留mid，循环条件也要改成`left < right`。
