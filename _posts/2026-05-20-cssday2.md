...
| 阶段         | 层顺序                  | 未分层位置               |
| :----------- | :---------------------- | :----------------------- |
| normal       | 先声明 < 后声明         | 在**最后**（最高优先级） |
| `!important` | 先声明 > 后声明（反转） | 在**最前**（最低优先级） |
...
CSS 属性分为两类：**可继承的** 和 **不可继承的**。当一个元素**没有对某个属性显式设值**时，浏览器根据属性类型决定从哪里取值。

### Inherited properties（可继承属性）

没有显式设值时，元素从**父元素**获取该属性的计算值。只有文档根元素（`<html>`）才会获取属性的初始值（CSS 规范定义的默认值）。
...
没有显式设值时，元素获取的是 CSS 规范定义的**初始值**，不会去父元素找。
...
现在 “emphasized text” 也有边框了，`<em>` 强制从 ``继承了 `border: medium solid`。
...
| 类型     | 没设值时从哪取       | 常见属性                                                 |
| :------- | :------------------- | :------------------------------------------------------- |
| 可继承   | 父元素的**计算值**   | `color`、`font-*`、`text-*`、`line-height`、`visibility` |
| 不可继承 | CSS 规范的**初始值** | `border`、`margin`、`padding`、`width`、`display`        |
