...
### Token 深入：JWT 的两种 payload 策略

一个容易被追问的点是：JWT 的 payload 里到底放什么？放了完整用户信息，跟 Cookie 有什么区别？只放一个 ID，跟 Session 又有什么区别？

JWT 由三部分组成，用 `.` 连接：header（算法类型，base64 编码）、payload（数据，base64 编码）、signature（签名，用密钥对前两部分计算出的哈希值）。**"只验签名"**的含义是——服务端收到 JWT 后，拿 header + payload 用同样的密钥重新算一遍签名，跟发过来的对比。对上了，就说明 payload 没人动过。这不是因为数据加了密（base64 不是加密，任何人都能解码），而是因为**改了 payload 就算不出匹配的签名**。

根据 payload 放什么，JWT 的实际行为完全不同：

**策略一：payload 放完整用户信息**（`{userId, role, permissions, ...}`）

这是 JWT 最独特的场景。跟 Cookie 对比：数据都在客户端，但 Cookie 的内容可以被用户随意篡改（DevTools 里就能改），而 JWT 改了数据就过不了签名验证。所以同样是客户端存数据，Cookie 只能说"这是我记录的"，JWT 可以说"这是我的证据"。

跟 Session 对比：服务端完全不查库，验完签名直接信任 payload。一台断开数据库连接的新机器拿到 token 就能独立完成鉴权——这正是微服务间传身份、第三方 API 授权、一次性下载链接等场景的基础。

**策略二：payload 只放一个 ID**（`{userId: 123}`）

这时候服务端验完签名还是要查数据库拿用户信息，跟 Session 用 session_id 查 Redis 在逻辑层面没本质区别。但省掉的是**基础设施**——不需要 Redis 做 session 共享存储，不需要处理 session 过期清理，任何服务器拿到 token 独立验签名就完事。查 DB 本身是免不了的，但省掉了"共享一个 Redis 集群"的运维复杂度。

所以 JWT 真正的不可替代性不在"比 Cookie 存得多"或"比 Session 快"，而在于**自包含 + 防篡改**这一对组合：信息带在客户端身上、不依赖服务端存储、同时能保证信息没被改过。Cookie 能做到第一点和第二点但不能防篡改，Session 能做到第二点和第三点但信息在服务端。三者各安其位。
