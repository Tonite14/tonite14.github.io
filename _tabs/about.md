...
/* ==========================================================================
   8.5 MyGO!!!!! 考据问答模块
   ---------------------------------------------------------------------------
   设计规范：与名片组件共享金色（偶像大师风格）+ MyGO!!!!! 青色应援色，
   独立设计 token 作用域（.mygo-quiz），确保与名片视觉统一但结构解耦。
   字体链与全局 custom.css:root 字体 token 保持一致（Lato 优先）。
   ========================================================================== */
.mygo-quiz {
  /* ── 字体链（与名片一致） ── */
  --qz-font-body:    "Lato", "Source Sans Pro", -apple-system, BlinkMacSystemFont,
    "Segoe UI", Roboto, "PingFang SC", "Microsoft YaHei", sans-serif;
  --qz-font-quote:   "Noto Serif JP", "Lato", Georgia, serif;

  /* ── 金色调色板（复用名片色值） ── */
  --qz-gold:          #ffd700;
  --qz-gold-dark:     #b8860b;
  --qz-gold-border:   rgba(255, 215, 0, 0.45);
  --qz-gold-subtle:   rgba(255, 215, 0, 0.3);
  --qz-gold-faint:    rgba(255, 215, 0, 0.12);
  ...
  /* ── 阴影 ── */
  --qz-shadow:        0 20px 50px rgba(0, 0, 0, 0.35), 0 0 0 1px var(--qz-gold-subtle);
...
.quiz-frame {
  ...
  border-radius: 16px;
  background: linear-gradient(160deg, rgba(13, 59, 77, 0.92) 0%, rgba(10, 40, 55, 0.95) 100%);
...
.quiz-frame::before {
  ...
  position: absolute;
  inset: 7px;
  border: 1px solid var(--qz-gold-border);
  border-radius: 12px;
  pointer-events: none;
  z-index: 0;
}

/* ── 装饰角标（与名片 corner 统一风格） ── */
.quiz-corner {
  position: absolute;
  width: 18px;
  height: 18px;
...
.quiz-corner--tl { top: 12px; left: 12px; border-right: none; border-bottom: none; border-top-left-radius: 6px; }
.quiz-corner--tr { top: 12px; right: 12px; border-left: none; border-bottom: none; border-top-right-radius: 6px; }
.quiz-corner--bl { bottom: 12px; left: 12px; border-right: none; border-top: none; border-bottom-left-radius: 6px; }
.quiz-corner--br { bottom: 12px; right: 12px; border-left: none; border-top: none; border-bottom-right-radius: 6px; }
...
.quiz-badge {
  ...
  color: var(--qz-deep);
  background: linear-gradient(135deg, var(--qz-gold) 0%, var(--qz-gold-dark) 100%);
...
.quiz-progress-bar {
  height: 100%;
  width: 0%;
  background: linear-gradient(90deg, var(--qz-cyan) 0%, var(--qz-gold) 100%);
  border-radius: 2px;
...
.quiz-option {
  ...
  border: 1px solid var(--qz-gold-faint);
  border-radius: 8px;
  padding: 0.8rem 1rem;
  cursor: pointer;
  transition: all 0.25s ease;
...
.quiz-option:hover:not(:disabled) {
  border-color: var(--qz-gold-border);
...
.quiz-feedback.is-correct {
  ...
  border-left: 3px solid var(--qz-correct);
  color: #c8e6c9;
}

.quiz-feedback.is-wrong {
  background: rgba(229, 115, 115, 0.1);
  border-left: 3px solid var(--qz-wrong);
  color: #f0c4c4;
}
...
.quiz-result {
  ...
  animation: quiz-feedback-in 0.4s ease;
}
...
.quiz-score-text {
  ...
  font-weight: 900;
  background: linear-gradient(135deg, var(--qz-gold) 0%, #fff 50%, var(--qz-gold) 100%);
  -webkit-background-clip: text;
  background-clip: text;
  -webkit-text-fill-color: transparent;
...
@media (max-width: 768px) {
  .mygo-quiz { margin-top: 3rem; }
  .quiz-frame { padding: 2rem 1.5rem 1.8rem; }
}

@media (max-width: 576px) {
  .mygo-quiz { margin-top: 2.5rem; padding: 0 0.5rem; }
  .quiz-frame { padding: 1.6rem 1.2rem 1.5rem; }
...
@media (max-width: 420px) {
  .quiz-frame { padding: 1.4rem 1rem 1.3rem; }
  .quiz-corner { width: 14px; height: 14px; }
  .quiz-question { font-size: 1rem; }
  .quiz-option { padding: 0.6rem 0.8rem; font-size: 0.85rem; }
}
